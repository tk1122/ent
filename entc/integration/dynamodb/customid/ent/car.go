...
type CarItem struct {
	...
	BeforeID float64 `dynamodbav:"before_id"`
	AfterID  float64 `dynamodbav:"after_id"`
	Model    string  `dynamodbav:"model"`

	PetCars *string `dynamodbav:"pet_cars"`
}
...
func (c *Car) FromItem(item interface{}) error {
	...
	c.ID = carItem.ID
	c.BeforeID = carItem.BeforeID
	c.AfterID = carItem.AfterID
	c.Model = carItem.Model

	c.pet_cars = carItem.PetCars
