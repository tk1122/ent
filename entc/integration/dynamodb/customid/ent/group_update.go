...
func (gu *GroupUpdate) Save(ctx context.Context) (int, error) {
	...
	if len(gu.hooks) == 0 {
		affected, err = gu.dynamodbSave(ctx)
	} else {
...
func (gu *GroupUpdate) dynamodbSave(ctx context.Context) (n int, err error) {
	...
	if n, err = dynamodbgraph.UpdateNodes(ctx, gu.driver, _spec); err != nil {
		return 0, err
	}
	return n, nil
}
...
func (guo *GroupUpdateOne) dynamodbSave(ctx context.Context) (_node *Group, err error) {
	_spec := &dynamodbgraph.UpdateSpec{
		Node: &dynamodbgraph.NodeSpec{
			Table: group.Table,
			Keys:  group.Keys,
	...
	if fields := guo.fields; len(fields) > 0 {
		...
		_spec.Node.Keys = append(_spec.Node.Keys, group.FieldID)
	...
	_spec.Assign = func(item interface{}) error {
		...
		_node.config = guo.config

		return nil
	}
	if err = dynamodbgraph.UpdateNode(ctx, guo.driver, _spec); err != nil {
		return nil, err
	}
	return _node, nil
}
