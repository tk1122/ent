...
func (pd *PetDelete) Exec(ctx context.Context) (int, error) {
	...
	if len(pd.hooks) == 0 {
		affected, err = pd.dynamodbExec(ctx)
	} else {
...
func (pd *PetDelete) dynamodbExec(ctx context.Context) (int, error) {
	...
	return dynamodbgraph.DeleteNodes(ctx, pd.driver, _spec)
}
